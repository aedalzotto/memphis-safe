...
class Eval:
    def __init__(self, testcase, dataset):
        ...
        if testcase is not None:
            self.base_scenario = ["{}/{}".format(testcase, scenario) for scenario in listdir(testcase) if scenario.startswith("sc_") and scenario.endswith("_ht")]
            self.rtd_scenario  = ["{}/{}".format(testcase, scenario) for scenario in listdir(testcase) if scenario.startswith("sc_") and scenario.endswith("_ht_rtd")]
    ...
    def __get_duration(scenario, apps):
                ...
                for line in f:
                    if line[0] == "$":
                        ...
                        if tokens[0] == "App":
                            ...
                            elif tokens[2] == "terminated":
                                duration[int(tokens[1])] = int(tokens[5]) - beggining[int(tokens[1])]
            ...
            for app in apps:
                ...
                line["app"] = app
                line["duration"] = duration[int(app)]
                lines.append(line)
